...
class Program
{
    ...
    static string GenerateSql(Spec spec, Catalogs cat)
    {
        ...
        sb.AppendLine($"EXEC dbo.eliminarVista {Sql(v.IdVista)};");
        sb.AppendLine();
        ...
        sb.AppendLine($@"
                        ...
                        VALUES
                            (1, NULL, {Sql(v.IdVista)}, {Sql(v.Descripcion)}, SYSUTCDATETIME(), N'1.0', {Sql(v.ArchivoJs)}, @VistaId, {Sql(v.TipoVista)}, {(v.UsaSeguridad ? 1 : 0)}, {(v.StringsConnectionId ?? cat.StringsConnectionId)});
                        ");
        ...
        sb.AppendLine(@"
                        ...
                        BEGIN
                            ...
                            SELECT fe.IdUsuario, fe.IdVista
                            INTO #FavVista
                        ...
                        BEGIN
                            ...
                            SELECT fee.IdUsuario, e.Descripcion AS NombreEstado
                            INTO #FavEstado
        ...
        foreach (var e in spec.Estados)
        {
            sb.AppendLine($@"
                            ...
                            VALUES (@IdEstado, {(e.Default ? 1 : 0)}, {Sql(e.Descripcion)}, @VistaId, {Sql(e.ArchivoJs)}, 0);
                            ");
            if (!string.IsNullOrWhiteSpace(e.ConfigEstadosJson))
            {
                sb.AppendLine($@"
                                INSERT INTO dbo.ConfiguracionesEstados (IdEstado, Configuracion)
                                VALUES (@IdEstado, {Sql(e.ConfigEstadosJson)});
                                ");
            }
            ...
            foreach (var f in e.Filas ?? new())
            {
                sb.AppendLine($@"
                                ...
                                VALUES (@IdFila, @IdEstado, {Sql(f.Descripcion)});
                                ");
                ...
                foreach (var a in f.Areas)
                {
                    sb.AppendLine($@"
                                    ...
                                    VALUES (@IdArea, @IdFila, {Sql(a.Descripcion)});
                                    ");
                    ...
                    if (a.Extraccion is not null)
                    {
                        ...
                        sb.AppendLine($@"
                                        ...
                                        VALUES (@IdExtraccion, {Sql(a.Descripcion)});
                                        ...
                                        VALUES (@IdSP, {Sql(a.Extraccion.SpNombre)}, N'', 0, {(spec.Vista.StringsConnectionId ?? cat.StringsConnectionId)}, @IdExtraccion, {tipoEjec});
                                        ");
                    }
                    else
                    {
        ...
        sb.AppendLine(@"
                        ...
                        BEGIN
                            INSERT INTO dbo.FavoritosEngineNCA (IdUsuario, IdVista)
                            SELECT f.IdUsuario, @VistaId
                            FROM #FavVista f;
                        END

                        IF OBJECT_ID('tempdb..#FavEstado') IS NOT NULL
                        BEGIN
                            INSERT INTO dbo.FavoritosEstadoEngineNCA (IdUsuario, IdEstado)
                            SELECT f.IdUsuario, e.IdEstado
                            FROM #FavEstado f
